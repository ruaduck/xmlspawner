...
namespace Server.Mobiles
{
	...
	public class XmlSpawner : Item, ISpawner, ISpawnObjectFinderList
	{
		...
		public List<ISpawnObjectFinder> i_SpawnObjects { get{ return m_SpawnObjects.ToList<ISpawnObjectFinder>(); } }
		...
		public bool CanFreeSpawn
		{
			get
			{
				...
				if (Running && m_ProximityRange == -1 &&
					...
					(!m_ExternalTriggering || m_ExternalManualTrigger))
					return true;
				else
					return false;
			}
		...
		public string RegionName
		{
			...
			set
			{
				...
				if (Region.Regions.Count == 0)	// after world load, before region load
					return;
				Map map=this.Map;
				if(this.RootParent!=null)//inside a container or weared by someone?!
				{
					if(this.RootParent is Mobile)
						map=((Mobile)this.RootParent).Map;
					else if(this.RootParent is Item)
						map=((Item)this.RootParent).Map;
				}
				if(map==null || map==Map.Internal)
					return;					

				Region region;
				if(map.Regions.TryGetValue(m_RegionName, out region) && region!=null)
				{
					m_Region = region;
		...
		*/
		...
		{
			get { return m_ExternalManualTrigger; }
			set
			{
				if(value)
				{
			...
			get
			{
				if (m_refractActivated)
				{
					if(m_RefractoryTimer!=null && m_RefractoryTimer.Running && m_RefractoryTimer.Delay>TimeSpan.Zero)
						return m_RefractoryTimer.Delay;
				}
		...
		{
			get { return false; }
			set { if (value == true) LoadXmlConfig(Path.Combine(XmlConfigsDir, ConfigFile)); }
		}
	   ...
	   if(index < MaxTraces){
							...
							switch (keyvalueargs[0])
							{
								...
								case "#CONDITION":
									...
									if (ckeyvalueargs.Length > 1)
									{
										...
										string error;
										if (!BaseXmlSpawner.CheckPropertyString(this, this, ckeyvalueargs[1], m_mob_who_triggered, out error))
										{
											status_str = error;
											return false;
										}
		...
		public Point3D GetSpawnPosition(bool requiresurface, int packrange, Point3D packcoord, List<SpawnPositionInfo> spawnpositioning, Mobile mob)
		{
			...
			if (m_TmpLocations==null && (includetilelist != null || excludetilelist != null || tileflag != TileFlag.None))
			{
				...
				{
					FindRegionTileLocations(ref m_TmpLocations, m_Region, includetilelist, excludetilelist, tileflag, checkitems, this.Z);
				}
				else if (positioning == SpawnPositionType.Random)
				{
					FindTileLocations(ref m_TmpLocations, this.Map, m_X, m_Y, m_Width, m_Height, includetilelist, excludetilelist, tileflag, checkitems, this.Z);
				}
			}
