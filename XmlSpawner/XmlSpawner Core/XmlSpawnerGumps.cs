	...
	public class HelpGump : Gump
	{
			...
			: base(X, Y)
		{
			...
			AddHtml(25, 20, width - 10, 440,
				...
				"SETVALUE,namevar,value[,duration] (NUMBERS)\n" +
				...
				"GIVE[,probability (0.01=1% 1=100%)]/«itemtypetogive/property/valore/...»\n" +
				"TAKE[,probability (0.01=1% 1=100%)[,quantity[,searchinbank (true/false)[,itemtype]]]]/itemname\n" +
				...
				"SPAWN[,spawnername],subgroup\n" +
				"GOTO/subgroup\n" +
				"GOTOIF/condition/thensubgroup[/elsesubgroup]\n" +
				...
				"SEFFECT,itemid,duration,speed,color,rendermode[,x,y,z] OR SEFFECT,itemid,duration,speed,color,rendermode[,trigmob]\n" +
			...
			AddHtml(25, 20, width - 10, 440,
			    "ATTENTION, for all the GET* using, in a spawner, the ^ inside {} (example {^GETONTRIGMOB/Location.x}) will execute that GETTER after the\nmob/item will be spawned, making functional the GET* on the object, without the ^ in a spawned item the command will not work!\n\n" +
			    "property/@value\n\n" +
				"Special KeyWords\n" +
				...
				"MUSIC,musicname[,distance]\n" +
				"SOUND,value\n" +
				"PRIVSOUND,value (only on a single mob)\n" +
				"EFFECT,itemid,duration[,x,y,z]\n" +
				"BOLTEFFECT[,sound[,color]]\n" +
				"MEFFECT,itemid[,speed][,x,y,z][,x2,y2,z2]\n" +
				"PEFFECT,itemid,speed,[x,y,z]\n" +
				"SEFFECT,itemid,duration,speed,color,rendermode[,x,y,z]\n" +
				...
				"YELL[,probability (0.01=1% 1=100%)][,unhide (true of false - default false)]/text\n" +
				...
				"SETVAR,varname,value[,duration] (STRINGS!)\n" +
			...
			AddPage(5);
			AddLabel(27, 2, 0x384, "XMLSpawner spawn lines (one line one command, end of command with ;)");
			AddHtml(25, 20, width - 10, 440,
				"#TILES,start[,end] spawn on tileid inside range\n" +
				"#ITEMID,start[,end] as TILES, but valid on itemid (ITEMs)\n" +
				"#NOTILES,start[,end] opposite of TILES (no spawn on tileid)\n" +
				"#NOITEMID,start[,end] opposite of ITEMID (no spawn on itemid)\n" +
				"#WET spawn only on WATER\n" +
				"#XFILL,inc spawn on line X - 1 element every 'inc'\n" +
				"#YFILL,inc as XFILL but on Y axis\n" +
				"#EDGE,inc as XFILL but on spawn perimeter\n" +
				"#PLAYER,range spawn only if a player is in range (trig)\n" +
				"#WAYPOINT,name[,range] spawn on waypoint with name EQUAL, range optional\n" +
				"#RELXY,xinc,yinc[,zinc] spawn on relative XY[Z]\n" +
				"#XY,xinc,yinc[,zinc] spawn on exact position with indicated increments\n" +
				"#DXY,xinc,yinc[,zinc] spawn on delta (original spawn) with indicated increments\n" +
				"#CONDITION,propertytest spawn with check on trigmob (triggerer), then  spawn (this)\n",
				false, true);
