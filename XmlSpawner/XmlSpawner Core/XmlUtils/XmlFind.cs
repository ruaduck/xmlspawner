...
namespace Server.Mobiles
{
	...
	public class XmlFindGump : Gump
	{
		...
		public class XmlFindThread
		{
			...
			public XmlFindThread(Mobile from, SearchCriteria criteria, string commandstring, List<Item> ilist, List<Mobile> mlist)
			{
			...
			public void XmlFindThreadMain()
			{
				...
				List<SearchEntry> results = XmlFindGump.Search(m_IList, m_MList, m_SearchCriteria, out status_str);
				CommandLogging.WriteLine(m_From, "{0} {1} Results:{2} - {3}", m_From.AccessLevel, CommandLogging.Format(m_From), results.Count, m_SearchCriteria.ToString());
				...
				if(m_Started<DateTime.UtcNow)
						m_From.SendMessage("Search completed in: {0:F2} seconds", DateTime.UtcNow.Subtract(m_Started).TotalSeconds);
				XmlFindAttachment xf = XmlAttach.FindAttachment(m_From, typeof(XmlFindAttachment)) as XmlFindAttachment;
				if(xf!=null && !xf.Deleted)
				{
					xf.AddResults(gump);
				}
				else
		...
		public class SearchCriteria
		{
			...
			public override string ToString()
			{
				...
				if(Dosearchwithattach)
					sb.AppendFormat("{0} Attachment - ", Searchattachornot ? "With":"Without");
				if(Dosearchcondition && !string.IsNullOrWhiteSpace(Searchcondition))
					sb.AppendFormat("PropTest='{0}' - ", Searchcondition);
				if(Dosearchname)
					sb.AppendFormat("Name='{0}' - ", (Searchname == null ? "(-NULL-)" : string.IsNullOrWhiteSpace(Searchname) ? "(-EMPTY-)":Searchname));
				if(Dosearchattach)
				...
				sb.AppendFormat("{0}{1}{2}{3}{4}{5}{6}{7}", (Dosearchint ? "Internal " : ""),(Dosearchnull ? "Null ":""),(Dosearchfel ? "Fellucca ":""),(Dosearchtram?"Trammel ":""),(Dosearchilsh?"Ilshenar ":""),(Dosearchmal?"Malas ":""),(Dosearchtok?"Tokuno ":""),(Dosearchter?"TerMur ":""));
				sb.Append("- ");
				if(Dohidevalidint)
		...
		public static void Initialize()
		{
			...
			EventSink.BeforeWorldSave += new BeforeWorldSaveEventHandler(PreSave);
			EventSink.WorldSave += new WorldSaveEventHandler(Save);
		}

		private static void PreSave( BeforeWorldSaveEventArgs e )
		{
		...
		{
			Timer.DelayCall(new TimerCallback(PostSave));
		}
		...
		private static bool TestRange(object o, int range, Map currentmap, Point3D currentloc)
		{
			...
			if (o is Item)
			{
				...
				if (o is Mobile)
				{
					...
					if (mob.Map == currentmap) return (Utility.InRange(currentloc, mob.Location, range));
					else if(mob.LogoutMap!=null && mob.LogoutMap==currentmap) return (Utility.InRange(currentloc, mob.LogoutLocation, range));
		...
		private static bool TestRegion(object o, string regionname)
		{
			...
			if (o is Item)
			{
				...
				if (item.Parent != null && item.RootParent != null)
				{
					...
					{
						loc=((Mobile)item.RootParent).Location;
						map=((Mobile)item.RootParent).Map;
					}
					else if(item.RootParent is Item)
					{
						loc=((Item)item.RootParent).GetWorldLocation();
						map=((Item)item.RootParent).Map;
					}
				}
				if(map==null)
					return false;
				Region r;
				if(map.Regions.TryGetValue(regionname, out r) && r!=null)
		...
		private static bool TestAttach(Item i)
		{
			...
			if ( xls != null && xls.Count>0 ) return true;
		...
		private static bool TestAge(Item it, double age, bool direction)
		{
			...
			{
				// true means allow only mobs greater than the age
				if ((DateTime.UtcNow - it.LastMoved) > TimeSpan.FromHours(age)) return true;
			}
			else
			{
				// false means allow only mobs less than the age
		...
		private static bool TestAge(XmlAttachment xa, double age, bool direction)
		{
			...
			{
				// true means allow only mobs greater than the age
				if ((DateTime.UtcNow - xa.CreationTime) > TimeSpan.FromHours(age)) return true;
			}
			else
			{
				// false means allow only mobs less than the age
		...
		public static List<SearchEntry> Search(List<Item> itemarray, List<Mobile> mobilearray, SearchCriteria criteria, out string status_str)
		{
			status_str = null;
			...
			if (!criteria.Dosearchattach)
			{
				...
				if (itemarray != null)
				{
					foreach (Item i in itemarray)
					{
						...
						{
							if(criteria.Searchname != null && (i.Name == null || i.Name.ToLower().IndexOf(criteria.Searchname.ToLower()) <= -1))
							 	continue;
						}
						...
						if (criteria.Dosearchwithattach)
						{
							if(TestAttach(i))
								hasattach = true;
							if((!criteria.Searchattachornot && hasattach) || (criteria.Searchattachornot && !hasattach)) continue;
						}
						...
						// check for entry
						if (criteria.Dosearchspawnentry || criteria.Dosearchspawntype)
						{
							...
							else
							{
								...
								else if (i is Spawner)
								{
									Spawner s = (Spawner) i;
									// search the entries of the spawner
									foreach (var so1 in s.SpawnObjects)
									{
										string so = so1.SpawnName;
										if (criteria.Dosearchspawntype)
										{
											// search by entry type
											Type type = null;
						...
						if ((criteria.Dosearchspawnentry || criteria.Dosearchspawntype) && !hasentry) continue;
			...
			if (criteria.Dosearchattach)
			{
				...
				foreach (XmlAttachment xa in foundtypes)
				{
					...
					if (criteria.Dosearchname)
					{
						if(criteria.Searchname != null && (xa.Name == null || xa.Name.ToLower().IndexOf(criteria.Searchname.ToLower()) <= -1))
							continue;
					}
					ISpawnable i=xa.AttachedTo as ISpawnable;
					...
					if(i!=null && !i.Deleted)
					{
						...
						if ((i.Map == Map.Felucca && criteria.Dosearchfel) || (i.Map == Map.Trammel && criteria.Dosearchtram) ||
							...
							(i.Map == null && criteria.Dosearchnull) || (i.Map == Map.Malas && criteria.Dosearchmal) || (tokunomap!=null && i.Map == tokunomap && criteria.Dosearchtok) || (i.Map == Map.TerMur && criteria.Dosearchter))
						{
							hasmap = true;
						}
		...
		[Description("Finds an object in the world - at the moment it is possible to recover an old search by inserting a digit from 1 to 3, the 3 is the older, 1 is the newer - example: xmlfind 1 - will show the most recent search")]
			...
			if (e.Arguments.Length > 0)
			{
				if(e.Arguments[0].Length==1)
				{
					if(int.TryParse(e.Arguments[0], out range))
					{
						...
						if(range>0 && range<4)
						{
							XmlFindAttachment xf = XmlAttach.FindAttachment(m, typeof(XmlFindAttachment)) as XmlFindAttachment;
							if(xf==null || xf.Deleted)
							{
								xf=new XmlFindAttachment();
							...
							m.SendMessage("At the moment it's possible to recover a maxium of 3 search, 3 is the older and 1 is the last search made!");
					}
					else
					{
						range=-1;
			...
			: base(X, Y)
		{
			...
			AddRadio(75, y, 0xD2, 0xD3, Sortowner, 6);
			AddLabel(98, y, 0x384, "owner");
			...
			AddCheck(65, y + 4, 0x938, 0x939, m_SearchCriteria.Searchattachornot, 331);
		...
		private void DoGoTo(int index)
		{
			...
			if (m_SearchList != null && index < m_SearchList.Count)
			{
				...
				{
					Item item = (Item)o;
					BaseXmlSpawner.GoToItem(m_From, item, true);
//					Point3D itemloc;
//					if (item.Parent != null)
//					{
//						if (item.RootParent is Mobile)
...
//					m_From.Map = item.Map;
					...
					{
						Mobile mob = (Mobile)o;
						BaseXmlSpawner.GoToMobile(m_From, mob, true);
...
//									m_From.Map = item.Map;
		...
		private void DoShowGump(int index)
		{
			...
			{
				object o = m_SearchList[index].Object;
				if (o is ISpawnObjectFinderList)
				{
					// dont open anything with a null map null item or deleted
					ISpawnObjectFinderList x = (ISpawnObjectFinderList)o;
		...
		private void DoShowProps(int index)
		{
			...
			if (m_SearchList != null && index < m_SearchList.Count)
			{
				...
				{
					Item x = (Item)o;
					if (x == null || x.Deleted || !x.CanTarget /*|| x.Map == null*/) return;
		...
		private void SortFindList()
		{
			if (m_SearchList != null && m_SearchList.Count > 0)
			{
				...
				{
					this.m_SearchList.Sort(new ListOwnerSorter(Descendingsort));
				}
			}
		}
		...
		private class ListOwnerSorter : IComparer<SearchEntry>
		{
			...
			public int Compare(SearchEntry e1, SearchEntry e2)
			{
				...
				{
					Item it=(Item)x;
					if(it.RootParent is Item)
						xstr = ((Item)it.RootParent).Name;
					else if(it.RootParent is Mobile)
						xstr = ((Mobile)it.RootParent).Name;
				}
				...
				if (y is Item)
				{
					Item it=(Item)y;
					if(it.RootParent is Item)
						ystr = ((Item)it.RootParent).Name;
					else if(it.RootParent is Mobile)
						ystr = ((Mobile)it.RootParent).Name;
				}
		...
		private void Refresh(NetState state)
		{
			state.Mobile.SendGump(new XmlFindGump(this.m_From, this.StartingLoc, this.StartingMap, false, this.m_ShowExtension, this.Descendingsort, this.m_SearchCriteria, this.m_SearchList, this.Selected, this.DisplayFrom, this.SaveFilename,
				...
				this.Sortmap, this.Sortselect, this.Sortowner, this.SelectAll, this.X, this.Y));
		}
			...
			if (text != null && text.Length > 0)
			{
				...
				if (args != null && args.Length >= 1)
				{
					...
					foreach (BaseCommand c in TargetCommands.AllCommands)
					{
						...
						{
							if (c.Commands[i].ToLower() == command.ToLower())
							{
				...
				case 3: // Search
					{
						...
						List<Mobile> mlist=null;

						if (!m_SearchCriteria.Dosearchattach)
							ilist=new List<Item>(World.Items.Values);
						//m_SearchList = Search(m_SearchCriteria, out status_str);
						if(!m_SearchCriteria.Dosearcherr && !m_SearchCriteria.Dosearchattach)
							mlist=new List<Mobile>(World.Mobiles.Values);
						...
						XmlFindThread tobj = new XmlFindThread(state.Mobile, m_SearchCriteria, CommandString, ilist, mlist);
		...
		public class XmlConfirmBringGump : Gump
		{
				...
				: base(0, 0)
			{
				...
				if (SearchList != null)
				{
					...
					{
						if (SearchList[i].Selected) count++;
					}
				}
							...
							for (int i = 0; i < SearchList.Count; i++)
							{
								...
								if (e.Selected)
								{
									...
									else if (o is XmlAttachment)
									{
										try
										{
											XmlAttachment xa=(XmlAttachment)o;
