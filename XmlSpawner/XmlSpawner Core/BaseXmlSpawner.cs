...
namespace Server.Mobiles
{
	...
	public class BaseXmlSpawner
	{
		...
		public static string SetPropertyValue(XmlSpawner spawner, object o, string name, string value)
		{
			...
			if (arglist.Length == 2)
			{
				...
				if (plookup != null)
				{
					...
					if (BaseXmlSpawner.IsProtected(type, propname, plookup))
				...
				else
				{
					...
					foreach (PropertyInfo p in props)
					{
						if (Insensitive.Equals(p.Name, propname))
						{
							...
							if (BaseXmlSpawner.IsProtected(type, propname, p))
								return "Property is protected.";
		...
		public static string SetPropertyObject(XmlSpawner spawner, object o, string name, object value)
		{
			...
			if (arglist.Length == 2)
			{
				...
				if (plookup != null)
				{
					...
					if (BaseXmlSpawner.IsProtected(type, arglist[0], plookup))
						return "Property is protected.";
				...
				else
				{
					...
					foreach (PropertyInfo p in props)
					{
						if (Insensitive.Equals(p.Name, arglist[0]))
						{
							...
							if (BaseXmlSpawner.IsProtected(type, arglist[0], p))
								return "Property is protected.";
			...
			else
			{
				...
				if (plookup != null)
				{
					...
					if (BaseXmlSpawner.IsProtected(type, name, plookup))
						return "Property is protected.";
				...
				else
				{
					foreach (PropertyInfo p in props)
					{
						if (Insensitive.Equals(p.Name, name))
						{
							...
							if (BaseXmlSpawner.IsProtected(type, name, p))
								return "Property is protected.";
		...
		public static bool ApplyObjectStringProperties(XmlSpawner spawner, string str, object o, Mobile trigmob, object refobject, out string status_str)
		{
			...
			if (string.IsNullOrEmpty(str) || o == null) return false;
			...
			if(str.IndexOf('}') != -1)//post spawn commands...
				str = ApplySubstitution(spawner, o, trigmob, true, str);
			else
				str = str.Trim();
			...
			while (arglist.Length > 1)
			{
				...
				if (arglist.Length > 1)
				{
						...
						arglist[0].Length > 0 && !Char.IsUpper(arglist[0][0]))
					{
						...
						//if(arglist[0].ToLower(Core.Culture) == "accesslevel")
						//{
...
//                                		singlearglist[1].Remove(0, 1);
					...
					else
					{
						if (IsValuemodKeyword(value_keywordargs[0]))
						{
							...
							else if (kw == valuemodKeyword.GETVALUE)
							{
								...
								if (value_keywordargs.Length > 1)
								{
									...
									XmlValue var = (XmlValue)XmlAttach.FindAttachment(refobject, typeof(XmlValue), varname);
							...
							else if (kw == valuemodKeyword.MUL)
							{
								...
								if (value_keywordargs.Length > 1)
								{
									...
									if (value_keywordargs.Length > 2)
									{
										...
										if(double.TryParse(value_keywordargs[1], NumberStyles.Float, CultureInfo.InvariantCulture, out d0) && double.TryParse(value_keywordargs[2], NumberStyles.Float, CultureInfo.InvariantCulture, out d1))
										{
									...
									else
									{
										...
										double d0,d1;
										if(double.TryParse(currentvalue, NumberStyles.Float, CultureInfo.InvariantCulture, out d0) && double.TryParse(incvalue, NumberStyles.Float, CultureInfo.InvariantCulture, out d1))
										{
						...
						else if (IsTypemodKeyword(keywordargs[0]))
						{
							...
							//  PRIVSOUND keyword
							//
							else if (kw == typemodKeyword.PRIVSOUND)
							{
								int sound = -1;
							...
							//  SEFFECT keyword
							//
							else if (kw == typemodKeyword.SEFFECT)
							{
								int effect = -1;
								int duration = 1;
								int speed=10;
							...
							else if (kw == typemodKeyword.MSG)
							{
								...
								if (keywordargs.Length > 1)
								{
									if(!double.TryParse(keywordargs[1], NumberStyles.Float, CultureInfo.InvariantCulture, out drop_probability))
							...
							else if (kw == typemodKeyword.YELL)
							{
								// if the object is a mobile then display a msg over the mob
								double drop_probability = 1;
								bool unhide = false;
							...
							else if (kw == typemodKeyword.SETVALUE)
							{
								...
								if (keywordargs.Length > 2)
								{
									...
									if(keywordargs.Length>3)
									{
										double.TryParse(keywordargs[3], NumberStyles.Float, CultureInfo.InvariantCulture, out duration);
									}
									...
									else
									{
										if(duration>0)
											...
											XmlAttach.AttachTo(o, new XmlValue(keywordargs[1], val));
									}
								}
							...
							else if (kw == typemodKeyword.SETVAR)
							{
								...
								if (keywordargs.Length > 2)
								{
									...
									XmlLocalVariable x = (XmlLocalVariable)XmlAttach.FindAttachment(o, typeof(XmlLocalVariable), keywordargs[1]);
									if(x!=null)
									{
										x.Data = val;
										if(duration>0)
									...
									{
										if(duration>0)
											XmlAttach.AttachTo(o, new XmlLocalVariable(keywordargs[1], val, duration));
										else
											XmlAttach.AttachTo(o, new XmlLocalVariable(keywordargs[1], val));
									}
								}
...
//                                		singlearglist[1].Remove(0, 1);
			...
			if ((startc == '.') || (startc == '-') || (startc == '+') || (startc >= '0' && startc <= '9'))
			{
				...
				if (startc == '"' || startc == '(')
				{
					...
					else if (IsValueKeyword(pname))
					{
						...
						else if ((kw == valueKeyword.GETVALUE) && arglist.Length > 1)
						{
							// syntax is GETVALUE,varname
							string varname = arglist[1];
							...
							// look for the xmllocalvariable attachment with the given name
							XmlValue var = (XmlValue)XmlAttach.FindAttachment(o, typeof(XmlValue), varname);
						...
						if ((ptype2 == typeof(double)) && IsNumeric(ptype1))
						{
							...
							if ((ptype1 == typeof(double)) && IsNumeric(ptype2))
							{
								...
								if ((ptype1 == typeof(double)) && (ptype2 == typeof(double)))
								{
									...
									if (hasequal)
									{
										if(double.TryParse(value1, NumberStyles.Float, CultureInfo.InvariantCulture, out val1) && double.TryParse(value2, NumberStyles.Float, CultureInfo.InvariantCulture, out val2))
										{
											if(val1==val2)
												return !invertreturn;
										}
		...
		{
			return ApplySubstitution(spawner, o, trigmob, false, typeName);
		}

		public static string ApplySubstitution(XmlSpawner spawner, object o, Mobile trigmob, bool postmod, string typeName)
		{
			...
			while (remaining != null && remaining.Length > 0)
			{
				...
				if(!post)//what is not posticipated postspawn will be elaborated immediately!
				{
					keypart = ParseForKeywords(spawner, o, keypart.Trim(), trigmob, true, out ptype);
				...
				else//what is posticipated will be thrown back with all the delimiters!
				{
					sb.Append('{');
					sb.Append(keypart);
					sb.Append('}');
				...
				if (endindex + startindex + sum >= remaining.Length) break;
		...
		public static void BroadcastMessage(AccessLevel ac, int hue, int font, string message)
		{
			int number;
			Packet p=null;
			if(int.TryParse(message, out number) && number>0)
	...
	string propertyString, out string status_str)
		...
		*/
							...
							if(arglist.Length > 1)
							{
								...
								if(objecttype != null)
								{
									...
									if (!userange)
									{
										...
										{
											IEntity entity = invoker as IEntity;
											if (entity != null && entity.Map != null && entity.Map != Map.Internal)
											{
					...
					case typeKeyword.SETVAR:
						{
							...
							if (arglist.Length > 0)
							{
								...
								if(objstr.Length>3)
								{
									double.TryParse(objstr[3], NumberStyles.Float, CultureInfo.InvariantCulture, out duration);
								}
								...
								if (a == null)
								{
									...
									if(duration>0)
										XmlAttach.AttachTo(invoker, new XmlLocalVariable(varname, varval, duration));
									else
					...
					case typeKeyword.SETVALUE:
						{
							...
							if (arglist.Length > 0)
							{
								...
								string varname = objstr[1];
								int val;
								int.TryParse(objstr[2], out val);
								XmlValue x = (XmlValue)XmlAttach.FindAttachment(invoker, typeof(XmlValue), varname);
								if(x!=null)
								{
									x.Value+=val;
									if(duration>0)
								...
								{
									if(duration>0)
										XmlAttach.AttachTo(invoker, new XmlValue(varname, val, duration));
									else
										XmlAttach.AttachTo(invoker, new XmlValue(varname, val));
								}
							}
							else
							{
					...
					case typeKeyword.WAITUNTIL:
						{
							...
							if (arglist.Length > 0)
							{
								...
								if (keywordargs.Length > 1)
								{
									if(!double.TryParse(keywordargs[1], NumberStyles.Float, CultureInfo.InvariantCulture, out delay))
										status_str = "invalid delay arg to WAITUNTIL";
								}
								if (keywordargs.Length > 2)
								{
									if (!double.TryParse(keywordargs[2], NumberStyles.Float, CultureInfo.InvariantCulture, out timeout))
					...
					case typeKeyword.EFFECT:
						{
							...
							if (arglist.Length > 0)
							{
								...
								if (keywordargs.Length > 3)
								{
                                    ...
                                    else if (keywordargs[3] == "trigmob" && triggermob != null)
                                    {
					...
					case typeKeyword.SEFFECT:
						{
							...
							if (arglist.Length > 0)
							{
								...
								if (keywordargs.Length > 6)
								{
                                    ...
                                    else if(keywordargs[6]=="trigmob" && triggermob!=null)
                                    {
                                        eloc = triggermob.Location;
                                        emap = triggermob.Map;
                                    }
		...
		public static void PlaySound( Mobile m, int soundID )
		{
			if ( soundID <= -1 || m==null || m.NetState==null )
				return;

			m.NetState.Send( new PlaySound( soundID, m.Location ) );
		}

		public static void GoToItem( Mobile from, Item item, bool log )
		{
			Map map = item.Map;
			...
			if(log)
				CommandLogging.WriteLine( from, "{0} {1} using GoTo on {2} \"{3}\"", from.AccessLevel, CommandLogging.Format( from ), CommandLogging.Format( item ), (owner!=null ? string.Format("{0} at {1} - {2}", CommandLogging.Format( owner ), loc, map) : string.Format("{0} - {1}", loc, map) ) );
			from.MoveToWorld( loc, map );
		}

		public static void GoToMobile( Mobile from, Mobile m, bool log )
		{
			Map map = m.Map;
			...
			if(log)
				CommandLogging.WriteLine( from, "{0} {1} using GoTo on {2} \"{3} - {4}\"", from.AccessLevel, CommandLogging.Format( from ), CommandLogging.Format( owner ), owner.Location, owner.Map );
			from.MoveToWorld( loc, map );
		}
